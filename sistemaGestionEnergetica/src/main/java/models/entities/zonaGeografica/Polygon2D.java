...
public class Polygon2D
{
	...
	public Point2D getPuntoMedio( Point2D p1, Point2D p2 )
	{
		double mediox = ( p1.getX() + p2.getX()) / 2 ;
		double medioy = ( p1.getY() + p2.getY()) / 2 ;
		
		return new Point2D.Double(mediox, medioy);
	}
	
	public Point2D getPuntoInterior()
	{
		Point2D puntoInterior = new Point2D.Double() ;
		
		for ( int i = 0 ; i < xpoints.length - 2 ; i ++)
		{
			
			Point2D punto1 = new Point2D.Double(xpoints[i], ypoints[i]) ;
			Point2D punto2 = new Point2D.Double(xpoints[i+1], ypoints[i+1]) ;
			
			Point2D puntoMedio = getPuntoMedio(punto1, punto2) ;
			
			Point2D punto3 = new Point2D.Double(xpoints[i+2], ypoints[i+2]) ;
			
			Point2D puntoInteriorMaybe = getPuntoMedio( puntoMedio, punto3) ;
			
			if ( this.contains( puntoInteriorMaybe ) )
			{
				puntoInterior = puntoInteriorMaybe ;
